    ...
    var a = $("#changePwd").modal({
        closeViaDimmer: 0, onConfirm: function (b) {
            ...
            if (o != "" && f != "" && f == d) {
                ...
                var c = e.encrypt(f);
                $.ajax({
                    url: "http://localhost:8080/regcheckpwd",
                    type: "post",
...
function createShare(chooseNum) {
    ...
    $.ajax({
        url: "http://localhost:8086/share/share",
        type: "post",
        contentType:'application/json;charset=UTF-8',
        data: JSON.stringify({
            "uid": $("#uid").val(),
        ...
        success: function (data) {
            if (data.respData.indexOf(",") > -1) {
                ...
                $(".share-url").val("http://localhost:8086/share/s/" + data.respData.substring(0, data.respData.lastIndexOf(",")) + "/" + $("#uid").val());
                $(".share-password").val(data.respData.substring(data.respData.lastIndexOf(",") + 1, data.respData.length));
                $(".copyPrivate").val("http://localhost:8086/share/s/" + (data.respData.substring(0, data.respData.lastIndexOf(","))) + "/" + $("#uid").val() + " 提取密码:" + data.respData.substring(data.respData.lastIndexOf(",") + 1, data.respData.length))
            } else {
                $(".share-validity-tip").css("right", "115px");
                $(".create-link").addClass("public-link has-create");
                $("#createShare").css("display", "none");
                $("#cel").children("span").children("span").html("关闭");
                $(".share-url").val("http://localhost:8086/share/s/" + data.respData)
            }
        },
        error: function () {
            alert("服务器错误")
