...
function checkUsername(flag) {
    if ($("#registForm input[name='username']").val() != "") {
		...
		$.get("http://localhost:8080/checkUsername?userName=" + $("#registForm input[name='username']").val(), function(data) {
			if (data.respCode == 0) {
				$("#TANGRAM__PSP_3__userNameError").html(
					data.respMsg);
				return false;
...
function checkPhone(flag) {
    if ($("#inputPhone").val() != "") {
        ...
        $.get("http://localhost:8080/checkPhone?phone=" + $("#inputPhone").val(), function (data) {
            if (data.respCode == 0) {
                $("#TANGRAM__PSP_3__phoneError").html(
                    data.respMsg);
                return false;
